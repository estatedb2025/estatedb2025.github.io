...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    // VIP用户数据规则
    match /vipusers/{vipId} {
      allow read: if request.auth != null; // 允许登录用户查询VIP身份
      allow create: if request.auth != null && vipId == request.auth.uid; // 允许用户创建自己的VIP记录
      allow update, delete: if isAdmin(request.auth.uid); // 仅管理员可设置VIP
    }

    // 需求文章(tasks)规则
    match /tasks/{taskId} {
      allow read: if true; // 所有人可读取任务
    ...
    function isVIP(uid) {
      // 检查用户是否在VIP集合中
      // 检查当前时间是否在VIP结束日期之前
      return exists(/databases/$(database)/documents/vipusers/$(uid)) && 
             get(/databases/$(database)/documents/vipusers/$(uid)).data.vipEndDate > request.time;
    }
  }
}
